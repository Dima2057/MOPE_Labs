...
while check:
    ...
    s = [0, 0, 0, 0]
    for i in range(m):
        s[0] += ((random_matrix_y[0][i] - y1[0]) ** 2) / 3
        s[1] += ((random_matrix_y[1][i] - y1[1]) ** 2) / 3
        s[2] += ((random_matrix_y[2][i] - y1[2]) ** 2) / 3
        s[3] += ((random_matrix_y[3][i] - y1[3]) ** 2) / 3
